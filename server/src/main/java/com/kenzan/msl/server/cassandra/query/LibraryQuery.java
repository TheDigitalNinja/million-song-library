...
public class LibraryQuery {
     ...
     * @return MyLibrary
     */
    public static MyLibrary get (final QueryAccessor queryAccessor, final MappingManager manager,final String sessionToken) {
     ...
     */
    private static List<AlbumInfo> getMyLibraryAlbums (final QueryAccessor queryAccessor,final MappingManager manager, final String uuid) {

        Result<AlbumsByUserDao> results = manager.mapper(AlbumsByUserDao.class).map(queryAccessor.albumsByUser(UUID.fromString(uuid)));
     ...
     */
    private static List<ArtistInfo> getMyLibraryArtists (final QueryAccessor queryAccessor, final MappingManager manager, final String uuid) {

        Result<ArtistsByUserDao> results = manager.mapper(ArtistsByUserDao.class).map(queryAccessor.artistsByUser(UUID.fromString(uuid)));
     ...
     */
    private static List<SongInfo> getMyLibrarySongs (final QueryAccessor queryAccessor, final MappingManager manager, final String uuid) {

        Result<SongsByUserDao> results = manager.mapper(SongsByUserDao.class).map(queryAccessor.songsByUser(UUID.fromString(uuid)));
